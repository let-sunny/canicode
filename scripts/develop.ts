...
function fetchIssue(issueNumber: number): IssueData {
  // Allow injecting issue data via env var when gh CLI is not available
  const injected = process.env["DEVELOP_ISSUE_JSON"];
  if (injected) {
    const data = JSON.parse(injected) as { number: number; title: string; body: string; labels?: Array<{ name: string } | string> };
    return {
      number: data.number,
      title: data.title,
      body: data.body,
      ...
      if (plan.split && plan.remainingDescription) {
        ...
        try {
          ...
          const ghOutput = execSync(
            `gh issue create --title ${shellEscape(`${conventionalizeTitle(issue.title)} (part 2)`)} --body ${shellEscape(followUpBody)}`,
            { encoding: "utf-8", cwd: PROJECT_ROOT },
          ).trim();
...
Generated by \`scripts/develop.ts\` ([#247](https://github.com/let-sunny/canicode/issues/247))`;
      ...
      const prTitle = conventionalizeTitle(issue.title);

      // Create draft PR (skip if SKIP_PR_CREATE=1 — create PR manually via MCP)
      let prUrl = "";
      if (process.env["SKIP_PR_CREATE"] === "1") {
        const prMeta = JSON.stringify({ title: prTitle, body: prBody, branch: index.branch }, null, 2);
        writeFileSync(join(runDir, "pr-meta.json"), prMeta + "\n");
        prUrl = "PENDING (create via MCP — see pr-meta.json)";
        console.log(`  [cli] Skipping gh pr create — pr-meta.json written for manual PR creation`);
      } else {
